...
func Auth_by_login_and_password(login, password string, auth_code int, reuse_token bool) (*govk.Api, error) {
	...
	if auth_info == nil {
		var err error
		auth_info, err = govk.Authenticate(login, password, CLIENT_ID, &SCOPE, auth_code)
		if err != nil {
			return nil, err
		}
		Add(login, auth_info)
...
func CheckAuthFlags(c *cobra.Command) error {
	login, password, access_token, _ := GetAuthFlags(c)
...
func Auth_by_flags(c *cobra.Command) (*govk.Api, error) {
	...
	login, password, access_token, auth_code := GetAuthFlags(c)
	...
	switch  {
		case access_token != "" :
			return Auth_by_access_token(access_token)
		case login != "" && password != "":
			return Auth_by_login_and_password(login, password, auth_code, reuse_token)
		default:
